...
var loadPositions = (function() {
  var createUpdateObjects = function( pipeObj, callback ) {
    ...
    for(var i=0; i < recs.length; i++ ) {
      ...
      docs.push( {query: { _id: id }, set: { $addToSet: { positions: recs[i].POS }}} );
    }
...
var inputSrc = { path: dataDir + '/Pos.csv',
              headers: 1,
          dataTypeMap: [ 'playerID', 'POS' ] };
...
loadPositions( pipeObj, function(err, result ) {
  if ( pipeObj.output ) {
    dataUtils.updateFns[ pipeObj.output.type ]( result.data[2], pipeObj.output, function( err, r) {
      console.log('[loadPositions] Positions loaded' );
    });
  }
});
